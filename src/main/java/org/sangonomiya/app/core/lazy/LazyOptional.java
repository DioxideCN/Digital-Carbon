...
/**
 * jdk自从1.8开始推出了经典的解决空指针异常的 {@link Optional} 类，现在我们对这个对象进行进一步的增强，使得它在<strong>线程安全或线程不安全</strong>的环境中都能拥有更强大的空指针处理能力（具体见 {@link #of(NonNullSupplier)} 方法），这就是你所看到的 {@link LazyOptional} 类。
 *
 * <p>LazyOptional 对象封装了一个特殊的 lazy 转换操作 (map/ifPresent) 得益于这种特性 LazyOptional 能分别处理<strong>线程安全和线程不安全</strong>环境下的方法返回值。
 *
 * <p>通过 {@link #addListener(NonNullConsumer)} 方法提供了一种监听失效活动的能力，当且仅当此对象的 Provider 调用了 {@link #invalidate()} 方法时才会调用此方法。
 *
 * <p>要创建此类的实例，必须使用 {@link #of(NonNullSupplier)} 并传递一个带有返回值的 {@link NonNullSupplier} 非空提供者参数，它可以是函数式接口、静态方法或内部代码块。
 *
 * <p>在 LazyOptional 中可以使用 {@link #empty()} 方法来检测空对象。
 *
 * <p>在下面给出的示例中演示了一个简单的 LazyOptional 使用方法
 *
 * <pre> {@code
 * String example = "full";
 * LazyOptional<String> lazy = LazyOptional.of(() -> {
 *     return example;
 * }).cast();
 * System.out.println(lazy.orElse("other choice"));
 * }</pre>
 *
 * @param <T> 被 LazyOptional 操作的方法返回类型
 *
 * @author Dioxide.CN
 * @date 2023/3/4 16:40
 * @since 1.0
...
public class LazyOptional<T> {
    ...
    // 1. null -> not resolved
    // 2. 非空 且包含 非空的值 -> resolved
    // 3. 非空 或包含 null -> resolved, 但 supplier 会返回 null (会导致冲突)
        ...
        if (resolved == null) {
            synchronized (lock) {
                ...
                if (resolved == null) {
                    T temp = supplier.get();
                    if (temp == null)
                        log.warn("Supplier should not return null value");
